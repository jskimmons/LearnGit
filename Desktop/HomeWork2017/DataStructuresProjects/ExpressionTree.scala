...
object ExpressionTree {
    ...
    var stackList : List[ExpressionTree] = Nil
    // method for part (a)
    def apply(expression : String) : ExpressionTree =  {
        ...
        for( x <- 0 to expression_Array.size-1) {
            if(operand.contains(expression_Array(x))){
                ...
                val left = stackList.head
                stackList = stackList.tail
                stackList = Node(expression_Array(x), left, right) :: stackList ::: Nil;
            }
            else{
                stackList = Leaf(expression_Array(x)) :: stackList ::: Nil;
            }
        }
        stackList.head;
    }
