 ...
 */
	...
	public void commandP(){
		for(int x=0;x<linetool.getArrayList().size();x++){	
			dictionary.convertIndicesBack(linetool.getArrayList().get(x));
		}
	}
	
	/**
	 * This handles the 'c' command which prints the current line from List lines after decompressing the file
	 */
	
	public void commandC(){
		dictionary.convertIndicesBack(linetool.getArrayList().get(LineTool.currentLine-1));
		System.out.println(dictionary.getStringBack().get(LineTool.currentLine-1));
	}
